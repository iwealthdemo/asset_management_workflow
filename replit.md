...
- July 19, 2025. Implemented comprehensive document scope control system for cross-document search functionality. Discovered that OpenAI vector stores do not support direct file_id filtering in Responses API - only metadata-based filtering is available. Implemented hybrid approach: Responses API for general searches with enhanced query instructions containing specific document names and file IDs, combined with Assistant API fallback using file attachments for precise document filtering when needed. System successfully constrains search results to user-selected documents through intelligent query enhancement and API-level filtering. Verified through comprehensive testing - document restriction working correctly for single documents, document subsets, and full document sets.
