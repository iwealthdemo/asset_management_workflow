...
import { CheckCircle, AlertTriangle, Clock, Eye, ChevronDown, ChevronUp, Edit, Send, FileText, Upload, X, Save, Search, Download } from "lucide-react";
...
export function InvestmentDetailsInline({ investment, isExpanded, onToggle }: InvestmentDetailsInlineProps) {
  ...
  const handleDownloadPDF = async (rationale: any) => {
    try {
      ...
      const pdfContent = `
Investment Rationale Report
${investment?.targetCompany || 'N/A'} - ${investment?.requestId || 'N/A'}

${rationale.content}

Generated by: ${rationale.author?.firstName} ${rationale.author?.lastName}
Created: ${new Date(rationale.createdAt).toLocaleDateString()}
Template: ${rationale.template?.name || 'N/A'}
`;
      
      // Create a simple text file download (can be enhanced to actual PDF later)
      const blob = new Blob([pdfContent], { type: 'text/plain' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Investment_Rationale_${investment?.targetCompany?.replace(/\s+/g, '_') || 'Unknown'}_${new Date().toISOString().split('T')[0]}.txt`;
      ...
      toast({ title: "Failed to download rationale", variant: "destructive" });
    }
  };
